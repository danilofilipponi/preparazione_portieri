...
export function createTacticalAuditDiagnostic(item:TacticalAuditItem):TacticalAuditDiagnostic{
  ...
  const remediationMatchCodes=new Set(["GK-MS-005","GK-MS-012","GK-MS-016","GK-MS-019","GK-MS-021","GK-MS-029","GK-MS-047","GK-TLR-040"]),multiPhaseFamily=(["REACTION","SECOND_BALL","DOUBLE_SAVE"] as string[]).includes(item.family)||item.family==="MATCH_SIMULATION"&&remediationMatchCodes.has(item.exercise.codice)?item.family as MultiPhaseFamily:null,multiPhasePlan=multiPhaseFamily&&supportsTacticalMultiPhasePlan(item.exercise,multiPhaseFamily)?createTacticalMultiPhasePlan(item.exercise,multiPhaseFamily):null;
