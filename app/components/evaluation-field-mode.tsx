...
export function EvaluationFieldMode({ sessionId, initialMode, onClose, onSessionChanged }: Props) {
  ...
  return <div ref={overlayRef} className="evaluation-field-overlay" role="dialog" aria-modal="true" aria-label="Modalit\u00e0 valutazione sul campo">
    ...
    <footer className="evaluation-field-footer"><button type="button" disabled={index === 0} onClick={() => setIndex(value => Math.max(0, value - 1))}>\u2190 Precedente</button><button type="button" className="evaluation-complete-button" onClick={() => setCompletionOpen(true)}>Termina valutazione</button><button type="button" disabled={index === payload.exercises.length - 1} onClick={() => setIndex(value => Math.min(payload.exercises.length - 1, value + 1))}>Esercizio successivo \u2192</button></footer>
...
function EvaluationResultsSummary({ payload, onClose }: { payload: EvaluationFieldPayload; onClose: () => void }) {
  ...
  return <div className="evaluation-results-overlay" role="dialog" aria-modal="true" aria-label="Risultati valutazione"><header><div><small>Risultati valutazione</small><h1>{payload.session.goalkeeper_name}</h1><p>{payload.session.evaluation_type === "Complete" ? "Valutazione completa" : "Valutazione mirata"} \u00b7 {new Date(`${payload.session.date}T12:00:00`).toLocaleDateString("it-IT")}</p></div><button onClick={onClose} aria-label="Chiudi risultati">\u00d7</button></header><main>
    {payload.session.evaluation_type === "Targeted" && <ParameterResults results={results}/>} 
