...
export function ExerciseCard({ exercise, onOpen, onEdit, onDeactivate, showActions = true, variant = "compact" }: ExerciseCardProps) {
  const procedureSteps = [exercise.schema_step_1, exercise.schema_step_2, exercise.schema_step_3, exercise.schema_step_4, exercise.schema_step_5].filter((step): step is string => Boolean(step));
  ...
  if (variant === "compact") return <article className="technical-card catalog-card">
    <div className="catalog-card-visual">
      ...
      <span className={`phase-pill ${exercise.fase.toLowerCase()}`}>{exercise.fase}</span>
    </div>
    <div className="catalog-card-body">
      ...
      <div className="catalog-card-facts"><span>♙ <b>{exercise.portieri_min}–{exercise.portieri_max}</b></span><span>◷ <b>{exercise.durata_min} min</b></span><span className="catalog-stars">{"★".repeat(exercise.difficolta)}{"☆".repeat(3 - exercise.difficolta)}</span></div>
  ...
  return <article className="technical-card manual-sheet">
    ...
    <section className="manual-bottom">
      ...
      <div className="manual-performance">
        ...
        <div className="manual-rating"><small>Difficoltà</small><strong>{"★".repeat(exercise.difficolta)}<i>{"☆".repeat(3 - exercise.difficolta)}</i></strong></div>
        <Fact icon="♙" label="Portieri">{exercise.portieri_min}–{exercise.portieri_max}</Fact>
      </div>
    </section>
